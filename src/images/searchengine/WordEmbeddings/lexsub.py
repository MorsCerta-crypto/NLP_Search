...
import WordEmbeddings.tools as t
...
class LexSub(object):
    ...
    def __init__(self, word_vectors=None, n_substitutes=5,
        ...
        if word_vectors is None:
            self.word_vectors = Word2Vec.load(WORD2VEC_PATH).wv
        else:
    ...
    def get_candidates(self, word, POS):
        ...
        result = t.process_candidates(result, word)[:self.n_candidates]
    ...
    def lex_sub(self, word_POS, sentence):
        ...
        try:
            ...
            if sentence is None:
                ...
                context_words = t.get_words(sentence)
                # filter context words: exist in the word2vec vocab, not stop words
                context_words = list(filter(lambda c: self.word_vectors.get_vecattr(c, "count") != 0
                                            and c not in t.stopw,
                                            context_words))
                cand_scores = [self.get_substitutability(w, s, context_words) if self.word_vectors.get_vecattr(s, "count") != 0 else 0 for s in candidates]
